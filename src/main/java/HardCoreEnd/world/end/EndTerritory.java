...
import HardCoreEnd.random.ColorUtil;
...
public enum EndTerritory{
    ...
    private EndTerritory(int chunkSize, int height, int bottom, int tokenColor, TerritorySpawnGenerator spawn, TerritoryProperties properties, TerritoryEnvironment environment, TerritoryGenerator.ITerritoryGeneratorConstructor constructor){
     ...
     */
    public ChunkPos getStartPoint(int index){
        int chunkOffX = 0;
        ...
        return new ChunkPos(chunkOffX+chunkOffset, chunkOffZ+chunkOffset);
    }

    public long getHashFromIndex(int index){
        ChunkPos startPoint = getStartPoint(index);
        return Pos.at(startPoint.x*16+chunkSize*8, ordinal(), startPoint.z*16+chunkSize*8).toLong();
    }
    ...
    public Pos generateTerritory(ChunkPos startPoint, World world, Random rand, EnumSet<? extends Enum<?>> variations, boolean isRare){
        for(int chunkX = 0; chunkX < chunkSize; chunkX++){
            for(int chunkZ = 0; chunkZ < chunkSize; chunkZ++){
                world.getChunkFromChunkCoords(startPoint.x+chunkX, startPoint.z+chunkZ);
            }
        }
        ...
        int startX = 16*startPoint.x+(int)structureWorld.getArea().x2;
        int startZ = 16*startPoint.z+(int)structureWorld.getArea().y2;
    ...
    Pair<Pos, EndTerritory> findTerritoryCenter(double posX, double posZ){
        ...
        for(EndTerritory territory:values){
            if (Math.abs(Math.abs(posX)-(middleX+territory.chunkSize*8)) < (territory.chunkSize+chunksBetween)*8){
                ...
                return new Pair(Pos.at(xStart, 0, zStart), territory);
            }
    ...
    /*public static final HeeTest $debugTest = new HeeTest(){
        ...
        public void run(String...args){
            ...
            if (args.length == 0){
                ...
                ChunkPos coords = territory.getStartPoint(DragonUtil.tryParse(args[2], 0));
                HardcoreEnderExpansion.notifications.report("Start: "+(coords.x*16)+", "+(coords.z*16));
                HardcoreEnderExpansion.notifications.report("Center: "+(coords.x*16+territory.chunkSize*8)+", "+(coords.z*16+territory.chunkSize*8));
            }
