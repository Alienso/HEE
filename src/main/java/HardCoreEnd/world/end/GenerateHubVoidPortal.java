    ...
    private boolean generateVoidPortal(StructureWorld world, Random rand, boolean ignoreHeightChecks){
        ...
        for(int offX = -8; offX <= 8; offX++){
            for(int offZ = -8; offZ <= 8; offZ++){
                ...
                if (offDist < 3D)world.setBlock(x+offX, y-1, z+offZ, Blocks.END_PORTAL, Meta.voidPortalTravel);
                else if (offDist < 4D)world.setBlock(x+offX, y-1, z+offZ, Blocks.END_PORTAL_FRAME, offDist == 3D ? Meta.voidPortalFrameStorage : Meta.voidPortalFramePlain);
