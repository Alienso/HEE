...
import HardCoreEnd.random.Facing4;
...
public class TerritorySpawnGenerator{
    ...
    void generatePortalAt(StructureWorld world, Random rand, Pos pos, boolean isRare){
        for(int offX = -2; offX <= 2; offX++){
            for(int offZ = -2; offZ <= 2; offZ++){
                if (Math.abs(offX) <= 1 && Math.abs(offZ) <= 1)world.setAttentionWhore(pos.getX()+offX, pos.getY(), pos.getZ()+offZ, null);//new BlockInfo(Blocks.END_PORTAL, isRare ? Meta.voidPortalDisabled : Meta.voidPortalReturn));
                else if (MathUtil.distance(offX, offZ) <= 2.32D)world.setAttentionWhore(pos.getX()+offX, pos.getY(), pos.getZ()+offZ, null);//new BlockInfo(Blocks.END_PORTAL_FRAME, Meta.voidPortalFramePlain));
            }
        }
    }
    ...
    public static interface ISpawnPointFinder{
        void findSpawnPoint(StructureWorld world, Random rand, EndTerritory territory, Pos.PosMutable pos);
    }
