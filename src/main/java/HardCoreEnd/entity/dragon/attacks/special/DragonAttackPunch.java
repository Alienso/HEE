...
import HardCoreEnd.entity.dragon.attacks.special.event.CollisionEvent;
...
public class DragonAttackPunch extends DragonSpecialAttackBase {
    ...
    public void update() {
        ...
        if (phase == PHASE_PREPARATION) {
            ...
            Vec3d vec = new Vec3d(-tempTarget.posX, 0D, -tempTarget.posZ).normalize();
            dragon.targetX = vec.x * 55D * (1D + rand.nextDouble() * 0.25D) + rand.nextInt(25);
            dragon.targetY = 72 + rand.nextInt(12);
            dragon.targetZ = vec.z * 55D * (1D + rand.nextDouble() * 0.25D) + rand.nextInt(25);
            ...
            if (dragon.dragonPartHead.getDistanceSq(tempTarget) < 35D) {
