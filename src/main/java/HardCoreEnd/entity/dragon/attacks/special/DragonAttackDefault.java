...
public class DragonAttackDefault extends DragonSpecialAttackBase {
    ...
    public void update() {
        ...
        if (!stealthInProgress && dragon.target == null && (targetCooldown == 0 || --targetCooldown == 0)) {
            ...
            int attackChance = Math.min(170, (dragon.angryStatus ? 45 : 5) + getDifficulty() * 5 + Math.min(viablePlayers, 5) * 14 + (CommonProxy.opMobs ? 10 : 0) + (50 - (healthPerc >> 1)));
        ...
        if (!dragon.angryStatus) {
            if (!stealthInProgress && ++seesCheck > 20 && dragon.target == null) {
                ...
                seesCheck = 0;
            } else if (stealthInProgress && overrideTarget != null && seesCheck % 3 == 0 && dragon.dragonPartHead.getDistanceSq(overrideTarget) < 27D) {
        ...
        if (stealthInProgress && !isPassiveAttackDisabled(EntityBossDragon.ATTACK_FIREBALL))
            setDisabledPassiveAttacks(EntityBossDragon.ATTACK_FIREBALL);
        else if (!stealthInProgress && isPassiveAttackDisabled(EntityBossDragon.ATTACK_FIREBALL))
            setDisabledPassiveAttacks();
    }
