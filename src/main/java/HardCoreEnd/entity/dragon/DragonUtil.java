package HardCoreEnd.entity.dragon;
...
public final class DragonUtil{
    ...
    public static boolean canEntitySeePoint(EntityLivingBase entity, double x, double y, double z, double pointScale, boolean fromCenter){
        ...
        for(int a = 0; a < 4; a++){
            for(int b = 0; b < 2; b++){
                RayTraceResult mop = entity.world.rayTraceBlocks(new Vec3d(px, py+0.12F, pz), new Vec3d(x+rayX[a]*pointScale, y+(b == 0 ? -1D : 1D)*pointScale, z+rayZ[a]*pointScale));
                ...
                if (mop != null && mop.typeOfHit == RayTraceResult.Type.BLOCK){
                    ...
                    if (isBlockedArr[a*2+b]){
                        ...
                        if (!entity.world.getBlockState(new BlockPos(mop.getBlockPos().getX(), mop.getBlockPos().getY(), mop.getBlockPos().getZ())).isOpaqueCube())isBlockedArr[a*2+b] = false;
                    }
                }
            }
        }
    ...
    public static float rotateSmoothly(float sourceAngle, float targetAngle, float amount){
        return sourceAngle+MathUtil.clamp(MathHelper.wrapDegrees(targetAngle-sourceAngle), -amount, amount);
    }
    ...
    public static <T extends Entity> List<T> getClosestEntities(int maxAmount, Entity source, List<? extends T> list){
        ...
        for(T entity:list){
            ...
            entities.put(entity, source.getDistance(entity));
        }
    ...
    public static int getTopBlockY(World world, Block block, int x, int z, int startY){
        for(Pos.PosMutable pos = new Pos.PosMutable(x, startY, z); pos.y >= 0; pos.y--){
    ...
    public static void spawnXP(Entity entity, int amount){
        for(int a = amount; a > 0;){
            ...
            a -= split;
            entity.world.spawnEntity(new EntityXPOrb(entity.world, entity.posX, entity.posY, entity.posZ, a));
        }
    }
