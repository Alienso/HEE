...
public class FXClientProxy extends FXCommonProxy{
    ...
    public static final void spawn(Particle fx){
    ...
    public static final boolean isInRange(Particle fx, double dist){ // TODO optimize distant particles
        Minecraft mc = Minecraft.getMinecraft();
        return mc.getRenderViewEntity() != null && MathUtil.distanceSquared(mc.getRenderViewEntity().posX-fx.getBoundingBox().minX, mc.getRenderViewEntity().posY-fx.getBoundingBox().minY, mc.getRenderViewEntity().posZ-fx.getBoundingBox().minZ) <= dist*dist;
    }
    ...
    public void global(String particleName, double x, double y, double z, double motionX, double motionY, double motionZ){
        switch(particleName){
            //TODO find id smoke large
            //case "smoke": spawn(new EntitySmokeFX(world(), x, y, z, motionX, motionY, motionZ)); break;
            case "largesmoke": spawn(new ParticleSmokeLarge.Factory().createParticle(1,world(), x, y, z, motionX, motionY, motionZ)); break;
    ...
    public void itemTarget(ItemStack is, double startX, double startY, double startZ, final double targetX, final double targetY, final double targetZ, final float speedMultiplier){
        Particle fx = (is.getItemSpriteNumber() == 0 ?
...
*/
    ...
    public void instability(EntityItemInstabilityOrb orb){
        ...
        if (rand.nextInt(4) == 0){
            spawn(new Particle(orb.worldObj, orb.posX, orb.posY+0.25D, orb.posZ, motX, motY, motZ){{
